 ...
 * (Events listener polyfill for IE8)
 */
(function() {
	'use strict';
	...
	var produceID = function(){
		return "h" + (+new Date) + Math.round(Math.random()*1000000);
	};
	...
	window.history.store = {
		...
		init: function() {
			...
			if (window.history.sessionStorageSupported)
				...
				this.states = [];
			if (!this.states.forEach)
				this.states = [];
			// console.log("init history states : ", this.states);
		},
		current:function(){
	...
	if (!window.history.pushState) // EMULATED
	{
		...
		setState = function(type, data, title, url)
		{
			...
			if(this.location.path == oldLocation.path && this.location.hash !== oldLocation.hash && this.config.setHashEvent)
				fireStateEvent("sethash", this.state, type);
			else if(this.config.setStateEvent)
	...
	var popState = function(e) {
		...
		var stateData = e.state, history = window.history;
		if (history.forceState)
			history.state = stateData || null;
		...
		var tmp = history.location,	// old
			current = history.location = removeBasePath(parseURL(location.href)), // new
			state;

		if(history.config.hid && stateData && stateData.hid)	// try to get state from hid
			state = history.store.find(stateData.hid);
		...
		if (tmp.hash !== current.hash && history.config.hashChangeAlone)
		{
			e.stopImmediatePropagation();
	...
	var hashChange = function(e) {
		...
		{
			 // could be from back/next button or manual hash change in address bar.
			var state,
				data = null,
	...
	window.history.initialised = false;
	window.history.init = function(opt) {
		if(window.history.initialised)
			return;
		window.history.initialised = true;
		for (var i in opt)
			this.config[i] = opt[i];
		...
		if (!this.emulated)
		{
			...
			this.location = removeBasePath(parseURL(window.location.href));
		}
		else
