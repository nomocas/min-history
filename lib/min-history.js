...
(function() {
	...
	if (!window.history.pushState) // EMULATED
	{
		...
		setState = function(type, data, title, url)
		{
			...
			if (this.config.hid)
			{
				this.state = data = this.store[type](data, title, url).data;	// 'push' or 'replace' in store
	...
	else // NATIVE
	{
		...
		setState = function(type, data, title, url){
			...
			this.location = parseURL(url);
			if(this.config.hid)
				data = this.store[type](data, title, url).data; // push or replace in store
			if(this.forceState)
				this.state = data || null;

			var r = original.call(this, data, title, url);
			if(this.config.setStateEvent)
				fireStateEvent('setstate', this.state, type);
			return r;
		}
	...
	var popState = function(e) {
		...
		var tmp = window.history.location,	// old
			current = window.history.location = parseURL(location.href), // new
			state;
		...
		{
			// check url equality.
			if(current.href !== state.url)
				throw new Error("bad state from native popstate (url dont match) "+current.href+" - "+state.url);
